...
**What it checks:** 30-year mortality trend (1994–2024) for all 7 countries.
...
**What it checks:** Male vs female under-5 mortality rates (2024), and quantified gender gap using self-join.
...
- Self-join technique used to reshape row-level data for side-by-side comparison. At production scale, Conditional Aggregation (`MAX(CASE WHEN ...)`) would be more efficient (single table scan vs two).

### Q5: Income Group vs Child Outcomes (3 queries)

**What it checks:** Correlation between country income group and mortality rate, plus country-level comparison against regional average.
...
- Q5-2 demonstrates SUBQUERY + CASE WHEN; Q5-2b refactors the same logic using CTE to eliminate duplicate subqueries
