...
SQL analysis of UNICEF child development data across 7 Southeast Asian countries. Built as a portfolio project to demonstrate SQL skills, Star Schema design, and data quality awareness — with a focus on indicators relevant to [World Vision Foundation of Thailand](https://worldvision.or.th)'s mission areas.
...
| Step | Who | Details |
|------|-----|---------|
| Extract | Me | Downloaded CSV from [UNICEF Data Warehouse](https://data.unicef.org) |
...
The Python ETL script was written with AI assistance (Claude). I directed the scope, schema design, and indicator selection — the script execution was AI-generated. I can explain every design decision but did not write the Python code myself.
...
**What it checks:** 30-year mortality trend (1994–2024) for all 7 countries, with Decade-over-Decade percentage change calculated automatically.
...
- Uses CTE + Window Function (LAG) to compute % change per decade — instead of leaving the reader to calculate manually

### Q4: Gender Disparity (3 queries)

**What it checks:** Male vs female under-5 mortality rates (2024), and quantified gender gap using two approaches.
...
- Q4-2a uses self-join; Q4-2b uses Conditional Aggregation (MAX + CASE WHEN) — same result, but conditional aggregation scans the fact table once instead of twice, making it more efficient at scale

### Q5: Income Group vs Child Outcomes (5 queries)

**What it checks:** Correlation between country income group and mortality rate, plus country-level comparison against regional average using three progressively refined approaches.
...
- Three approaches to the same problem: Q5-2a (subquery) → Q5-2b (CTE + CREATE VIEW) → Q5-2c (Window Function AVG() OVER()) — each more concise than the last
- Q5-2b packages the analysis as a reusable Data Mart view for BI/dashboard tools

## SQL Techniques Used

**DDL & Data Modeling:**
- CREATE VIEW — reusable Data Mart for BI/dashboard connectivity

**Window Functions:**
- LAG() + OVER(PARTITION BY ... ORDER BY ...) — decade-over-decade % change
- AVG() OVER() — aggregate calculation without collapsing rows

**Query Optimization:**
- Conditional Aggregation (MAX + CASE WHEN) vs Self-Join — single-pass vs dual-scan comparison

**Core SQL:**
- CTE (WITH...AS) + CROSS JOIN
- SUBQUERY (scalar subquery in SELECT and CASE WHEN)
...
- IN() / NOT IN() for multi-value filtering
- ROUND() for decimal formatting
- Self-join with table aliases (composite key matching)
