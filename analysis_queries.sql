...
-- Q3: UNDER-5 MORTALITY RATE TREND (30-YEAR SPAN) WITH DECADE-OVER-DECADE CHANGE
-- Purpose: Track child survival progress across SE Asia + calculate % change per decade
-- Indicator: CME_MRY0T4 (deaths per 1,000 live births)
-- Technique: CTE + Window Function (LAG) for Decade-over-Decade comparison
-- JD Match: Data Architecture + Advanced Analytics
WITH trend_data AS (
    SELECT 
        dim_country.country_name,
        fact_observation.year,
        fact_observation.value AS current_rate
    FROM fact_observation
    INNER JOIN dim_country
        ON fact_observation.country_code = dim_country.country_code
...
SELECT 
    country_name,
    year,
    ROUND(current_rate, 2) AS deaths_per_1000_live_births,
    ROUND(LAG(current_rate) OVER(PARTITION BY country_name ORDER BY year), 2) AS prev_decade_rate,
    ROUND(((current_rate - LAG(current_rate) OVER(PARTITION BY country_name ORDER BY year)) / 
        LAG(current_rate) OVER(PARTITION BY country_name ORDER BY year)) * 100, 2) AS pct_change
FROM trend_data
ORDER BY country_name, year;
-- Result: 28 rows — each country shows rate + previous decade rate + % change
-- 1994 rows show NULL for prev_decade_rate and pct_change (no prior decade to compare)
-- Cambodia most dramatic: 117.98 → 18.41 (~84% total reduction over 30 years)
...
-- Q4-2a: Gender Gap calculation — Self-Join approach
...
-- Performance note: self-join scans fact table twice — see Q4-2b for optimized version

-- Q4-2b: Gender Gap calculation — Conditional Aggregation (optimized)
-- Purpose: Same result as Q4-2a, but scans fact table only once
-- Technique: MAX(CASE WHEN ...) pivots M/F into columns within a single GROUP BY pass
-- DE note: preferred approach at scale — avoids duplicate table scan of self-join
SELECT 
    dim_country.country_name,
    ROUND(MAX(CASE WHEN fact_observation.sex_code = 'F' THEN fact_observation.value END), 2) AS female_rate,
    ROUND(MAX(CASE WHEN fact_observation.sex_code = 'M' THEN fact_observation.value END), 2) AS male_rate,
    ROUND(MAX(CASE WHEN fact_observation.sex_code = 'M' THEN fact_observation.value END) - 
          MAX(CASE WHEN fact_observation.sex_code = 'F' THEN fact_observation.value END), 2) AS gender_gap
FROM fact_observation
INNER JOIN dim_country
    ON fact_observation.country_code = dim_country.country_code
WHERE fact_observation.indicator_code = 'CME_MRY0T4'
    AND fact_observation.year = 2024
    AND fact_observation.sex_code IN ('M', 'F')
GROUP BY dim_country.country_name
ORDER BY gender_gap DESC;
-- Result: identical to Q4-2a — 7 rows, same values
-- Comparison: self-join is more readable; conditional aggregation is more efficient
-- Both are valid — choice depends on data volume and team preference
...
-- Q5-2a: Countries above/below regional average — SUBQUERY approach
...
-- Note: subquery is written twice (SELECT + CASE WHEN) — see Q5-2b/Q5-2c for cleaner versions
SELECT 
    dim_country.country_name,
...
-- Q5-2b: Same analysis — CTE + CREATE VIEW (Data Mart for BI)
-- Purpose: Refactor Q5-2a into a reusable view for dashboard/BI tools
-- Technique: CTE eliminates duplicate subquery + CREATE VIEW packages it as a data asset
-- JD Match: Dashboard & Reporting Enablement — DE prepares data, analyst consumes it
CREATE VIEW IF NOT EXISTS vw_regional_mortality_benchmark AS
WITH regional AS (
...
WHERE fact_observation.indicator_code = 'CME_MRY0T4'
    AND fact_observation.year = 2024
    AND fact_observation.sex_code = '_T';
-- To query: SELECT * FROM vw_regional_mortality_benchmark ORDER BY mortality_rate DESC;
-- Result: identical to Q5-2a — 7 rows, same values
-- Advantage: reusable view — BI tools can connect directly without rewriting SQL

-- Q5-2c: Same analysis — Window Function approach
-- Purpose: Demonstrate AVG() OVER() as the most concise solution
-- Technique: Window Function calculates aggregate without collapsing rows
-- No subquery, no CTE, no CROSS JOIN needed
SELECT 
    dim_country.country_name,
    ROUND(fact_observation.value, 2) AS mortality_rate,
    ROUND(AVG(fact_observation.value) OVER(), 2) AS regional_avg,
    CASE 
        WHEN fact_observation.value > AVG(fact_observation.value) OVER() THEN 'Above Average'
        ELSE 'Below Average'
    END AS status
FROM fact_observation
INNER JOIN dim_country
...
WHERE fact_observation.indicator_code = 'CME_MRY0T4'
    ...
    AND fact_observation.sex_code = '_T'
ORDER BY mortality_rate DESC;
-- Result: identical to Q5-2a and Q5-2b — 7 rows, same values
-- Progression: Q5-2a (subquery) → Q5-2b (CTE/VIEW) → Q5-2c (Window Function)
-- Each solves the same problem with increasing efficiency and readability
